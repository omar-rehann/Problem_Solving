...
function missingWord(nums, str) {
    ...
    // حوّل النص لمصفوفة حروف
    let chars = Array.from(cleanStr);

    // حاول تجيب الحروف
    let word = "";
    for (let i = 0; i < sorted.length; i++) {
        if (sorted[i] >= chars.length) {
            return "No mission today";
        }
        word += chars[sorted[i]].toLowerCase();
    }
...
console.log(missingWord([1, 0, 97], "omar rehan")); // No mission today